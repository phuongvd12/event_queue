...
uint8_t ELSTER_Send1Cmd_Test (void)
{
    ...
    Send_Data_Meter(&UART_METER,&ELSTER_Handshake1[0],5,1000);
    if (osSemaphoreWait(bsUART2PendingMessHandle,2000) == osOK) 
    {
        osDelay(ELSTER_Meter_Delay_ms);
        Send_Data_Meter(&UART_METER,&ELSTER_END[0],5,1000);
    }
	...
	while (Meter_Handshake.Reading_ui8 == 1)
	{
		switch (Meter_Handshake.Step_ui8)
		{
			...
			case 2:	//Send Handshake 2
				...
				Meter_Handshake.Step_ui8++;
				osDelay(ELSTER_Meter_Delay_ms);
				Send_Data_Meter(&UART_METER,&ELSTER_Handshake2[0],6,1000);
				break;
			case 3:
				if (osSemaphoreWait(bsUART2PendingMessHandle,5000) == osOK)
				{
			...
			case 4:	//Encrypt pass
				...
				Meter_Handshake.Step_ui8++;
				osDelay(ELSTER_Meter_Delay_ms);
				Send_Data_Meter(&UART_METER,&ELSTER_Handshake3[0],24,1000);
				break;
			case 5:
				if (osSemaphoreWait(bsUART2PendingMessHandle,5000) == osOK)
					Meter_Handshake.Step_ui8++;
...
uint8_t ELSTER_Get_Meter_ID (uint32_t TempValue)
{
	...
	while (GetMeterIDRetry--)
	{
		...
		{
			osDelay(500);
			Send_Data_Meter(&UART_METER,&ELSTER_Get_MID[0],16,1000);
...
void ELSTER_Send_MInfo_Mess (void)
{
	...
	Send_Data_Meter(&UART_METER,mMess,ELSTER_MInfo_Table[temp][22],1000);
}
...
void Send_MBilling_Mess (void)
{
	...
	Send_Data_Meter(&UART_METER,ELSTER_MeterBillingTemplate,16,1000);
}
...
void Send_MEvent_Mess (void)
{
	...
	Send_Data_Meter(&UART_METER,mMess,16,1000);
}
...
void Send_MLoadProfile_Mess (void)
{
	...
	switch (Get_Meter_LProf.Mess_Step_ui8)
	{
		case 0:
			...
			Send_Data_Meter(&UART_METER,RMLP_First_Mess,18,1000);
			break;
		case 1:
			Send_Data_Meter(&UART_METER,RMLP_Second_Mess,16,1000);
			break;
		default:
			...
			Send_Data_Meter(&UART_METER,RMLP_GetData_Mess,16,1000);
			break;
	}
}
