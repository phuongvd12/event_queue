...
void _fSET_SERVER_INFO (truct_String *str_Receiv)
{
    ...
    if(str_Receiv->Length_u16 > 27)     // 172.16.11.156:2000   b�nh thuong ca dau cach la 19 byte.
    {
...
{
    //dung thuc thi c�c task. Reset thiet bi
    DCU_Respond(PortConfig, (uint8_t *)"OK",2);
...
uint8_t CheckATCommandPC(void)
{
	...
	for (var = _ACCOUNT; var <= _QUERY_READ_INDEX_LOG; var++) 
    {
		...
		{
            //kiem tra xem c� 2 k� tu CR va LF cuoi cung khong thi xoa di
            ...
            Tempstring.Length_u16 = Uart1_Control.str_Receiv.Length_u16 - Pos_Str; // CR|LF // CR|LF. khi test k c� hai ki tu nay
			PortConfig = 1;
